define([
  ...
  var Cannon = function (camera, scene, input, audioContext, audioBuffer) {
    ...
    this.geometry = new THREE.CylinderGeometry(0.5, 0.5, lenght, 20, 1, false);
    this.geometry.applyMatrix( new THREE.Matrix4().makeTranslation( 0, length / 2, 0 ) );
    this.geometry.applyMatrix( new THREE.Matrix4().makeRotationX( Math.PI / 2 ) );
    this.material = new THREE.MeshPhongMaterial({ color: 0xff3730 });
  ...
  Cannon.prototype = {
    ...
    fire: function () {
      ...
      var source = this.audioContext.createBufferSource(); // Crea la sorgente audio
      source.buffer = this.audioBuffer;                    // seleziona l'audio da riprodurre
